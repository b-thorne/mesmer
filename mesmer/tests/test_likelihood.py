...
@pytest.fixture(params=list(product([-3., -2.9], [1.5, 1.6])), autouse=True, scope='function')
def fake_data(request):
    ...
    freq_obs = np.array([23., 30., 40., 95., 100., 145., 220., 270., 300., 353., 545, 800])
    ...
    return (freq_obs, request.param[0], request.param[1], templates, scaled_maps, data, covariance)
...
def test_max(fake_data):
    ...
    frequencies, true_beta_s, true_beta_d, templates, scaled_maps, data, covariance = fake_data
    ...
    logprob = LogProb(data=data, covariance=covariance, frequencies=frequencies, model=model)

    dlnP_dTheta = grad(logprob, 0)

    print('Log prob derivative Bd', dlnP_dTheta(true_params, True))
    print('Log prob derivative compared to logprob: ', dlnP_dTheta(true_params) / logprob(true_params))
    ...
    print("Fishers estimates of uncertainty: ", np.sqrt(fish_est[0, 0]), np.sqrt(fish_est[1, 1]))

    marg_errors =  np.sqrt(np.array([fish_est[0, 0], fish_est[1, 1]]))

    res = minimize(logprob, [true_beta_d, true_beta_s], tol=1e-9, args=(True), method='Powell')
    ...
    np.testing.assert_array_less(true_params - res.x, 5. * marg_errors)
    return